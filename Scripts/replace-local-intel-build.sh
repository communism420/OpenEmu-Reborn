...
if [[ "${1:-}" == --help || "${1:-}" == -h ]]; then
  ...
  echo "Reuses cores from the existing package by default. --cores explicitly replaces them with the supplied prebuilt set."
  echo "--cores is required for first publication and whenever newly built cores must replace old bundled cores."
