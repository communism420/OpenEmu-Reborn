...
- (id)init
{
	...
	_audioOutput->SetProducerRWLock(&rwlockCoreExecute);
...
{
	// DeSmuME's OpenEmu presenter currently renders at the native DS layout size.
	// Let OpenEmu scale that fixed IOSurface to the window instead of resizing the
	// backing OpenGL surface to the window; otherwise the native image is drawn in
	// the lower-left corner of a much larger black surface.
	return NO;
}
